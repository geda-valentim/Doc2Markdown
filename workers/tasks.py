...
def split_pdf_task(
    ...
    try:
        ...
        # Store total pages in parent job
        redis_client.set_job_pages(parent_job_id, total_pages)

        # Create PAGE JOBS para cada página
        for page_num, page_file_path in page_files:
            ...
            # Add page job as child of main job
            redis_client.add_child_job(parent_job_id, "page", page_job_id)

        # Mark split job as completed
        redis_client.set_job_status(
            job_id=split_job_id,
            job_type="split",
            status="completed",
...
def merge_pages_task(
    ...
    try:
        ...
        # Store merged result in main job
        redis_client.set_job_result(parent_job_id, merged_result)

        # Mark merge job as completed
        redis_client.set_job_status(
            job_id=merge_job_id,
            job_type="merge",
            status="completed",
